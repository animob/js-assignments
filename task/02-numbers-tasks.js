...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    let myScalar = (x1 * x2) + (y2 * y1);
    let myModule = (Math.sqrt(x1*x1 + y1*y1)) * (Math.sqrt(x2*x2 + y2*y2));
    return Math.acos(myScalar / myModule);
}
...
function roundToPowerOfTen(num, pow) {
    let myNumber = (Math.round(num / Math.pow(10,pow))) * Math.pow(10,pow);
    return myNumber;
}
